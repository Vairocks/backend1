...
.put(cors.corsWithOptions,authenticate.verifyUser,(req,res,next) => {
    Dishes.findById(req.params.dishId)
    .then((dish) => {
        if(dish!=null && dish.comments._id(req.params.commentId) != null && dish.comments.author==req.user._id){
            if(req.body.rating){
            ...
            else if(dish.comments.author==!req.user._id)
            {
                err = new Error('Sorry!, only a user can Edit his own comment');
                err.status=403;
                return(next(err));
            }
            else {
...
.delete(cors.corsWithOptions,authenticate.verifyUser,(req,res,next) =>{
    ...
    .then((dish) =>{
        console.log(dish);
        if(dish!=null && dish.comments.id(req.params.commentId) != null &&
             dish.comments.id(req.params.commentId).author.username==req.user.username){
        ...
        else if(dish.comments.id(req.params.commentId).author.username!=req.user.username)
        {
            err = new Error("Sorry the only user can delete his own comments");
            err.status=403;
            return(next(err));
        }
        else {
